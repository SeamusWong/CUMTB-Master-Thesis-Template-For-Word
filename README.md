...
2. *【此条已废弃，由于Word保存上的一些问题，文档已改为标准文档docx格式，对后续编辑没有影响可以照常试用】*！本文档格式为dotx，为Microsoft Word模板文件，当打开这个文件后，Word会按照模板创建一个新文档并且在保存的时候提示选择新文件位置，所以在此文件上的所有编辑均不会影响到原模板文件。
